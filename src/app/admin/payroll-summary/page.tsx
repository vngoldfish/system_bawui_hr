...
export default function PayrollSummaryPage() {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">会社負担 給与サマリー</h1>
      ...
      <div className="bg-white rounded-lg border p-6 mb-8">
        ...
        <table className="w-full">
          ...
          <tbody>
            <tr className="border-b"><td className="py-3">健康保険</td><td className="text-right">{formatCurrency(data.breakdown.healthInsurance)}</td><td className="text-right text-sm text-slate-500">{((data.breakdown.healthInsurance/data.totalCompanyCost)*100).toFixed(1)}%</td></tr>
            <tr className="border-b"><td className="py-3">厚生年金</td><td className="text-right">{formatCurrency(data.breakdown.pension)}</td><td className="text-right text-sm text-slate-500">{((data.breakdown.pension/data.totalCompanyCost)*100).toFixed(1)}%</td></tr>
            <tr className="border-b"><td className="py-3">雇用保険</td><td className="text-right">{formatCurrency(data.breakdown.employmentInsurance)}</td><td className="text-right text-sm text-slate-500">{((data.breakdown.employmentInsurance/data.totalCompanyCost)*100).toFixed(1)}%</td></tr>
            <tr className="border-b"><td className="py-3">労災保険</td><td className="text-right">{formatCurrency(data.breakdown.workersComp)}</td><td className="text-right text-sm text-slate-500">{((data.breakdown.workersComp/data.totalCompanyCost)*100).toFixed(1)}%</td></tr>
            <tr className="font-semibold bg-blue-50"><td className="py-3">合計</td><td className="text-right text-blue-700">{formatCurrency(data.totalCompanyCost)}</td><td className="text-right">100%</td></tr>
          </tbody>
        </table>
      </div>
